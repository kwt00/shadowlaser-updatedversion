...
def packCrsfToBytes(channels) -> bytes:
    # channels in CRSF format! (0-1984)
    # Values packed little-endianish: bits BA987654321 -> 87654321, 00000BA9
    ...
    newVal = 0
    for ch in channels:
        # Put low bits in any remaining dest capacity
        newVal |= (ch << destShift) & 0xff
        result.append(newVal)

        # Shift high bits down, place into the next dest byte
        ...
        # Next dest should shift up due to bits consumed
...
parser.add_argument('-P', '--port', default='/dev/tty.usbmodem203E345C57461', required=False)
parser.add_argument('-b', '--baud', default=152000, required=False)
