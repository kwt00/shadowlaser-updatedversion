...
SERIAL_PORT = "/dev/tty.usbmodem203E345C57461"  # Update to match drone hardware
BAUD_RATE = 115200

MSP_RAW_IMU = 102  # Cmd ID for MSP_RAW_IMU

ACCEL_SENSITIVITY = 2048.0  # LSB per g (+-16)
GYRO_SENSITIVITY = 16.4     # LSB per degree/second (for +-2000)
...
def parse_msp_raw_imu(data):
    ...
    # Unpack 9 integers, 2 bytes each, from raw data
        ...
        while True:
            ...
            if command == MSP_RAW_IMU and data is not None:
                ...
                if imu_data:
                    ...
                    print("Gyroscope (deg/s):", imu_data["gyroscope"])
                    #print("Magnetometer (raw):", imu_data["magnetometer"]) #Our drone doesn't have one
                    print()
            else:
